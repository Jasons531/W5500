...
uint8 check_DHCP_state(SOCKET s) 
{
  ...
  {   
    ...
    else  ///当读取数据失败时重复广播方式向DHCP服务器发送DISCOVER包   
    {
      printf("getSn_RX_RSR : %02x\r\n",getSn_RX_RSR(s));
      DHCP_timeout = 0;											// DHCP超时标志位清0
      reset_DHCP_time();										// 重置DHCP超时时间和重连次数
      send_DHCP_DISCOVER();									// 以广播方式向DHCP服务器发送DISCOVER包     
      DHCP_timer = 0;												// DHCP定时器清0  	
      dhcp_state = STATE_DHCP_DISCOVER;			// DHCP状态标志位置为STATE_DHCP_DISCOVER
    }
  }
  ...
  switch ( dhcp_state )											// DHCP状态转换过程
  {
    ...
    case STATE_DHCP_DISCOVER :
      if (type == DHCP_OFFER) 							// 接收到DHCP服务器发出的OFFER包
      {
        reset_DHCP_time();									// 重置DHCP超时时间和重连次数 
        send_DHCP_REQUEST();								// 向DHCP服务器发送REQUEST
        dhcp_state = STATE_DHCP_REQUEST;		// DHCP状态标志位置为STATE_DHCP_REQUEST
#ifdef DHCP_DEBUG			
        printf("state : STATE_DHCP_REQUEST\r\n");
#endif			
      }
      else 
        check_DHCP_Timeout();								// 检查DHCP是否超时
      break;
    
    case STATE_DHCP_REQUEST :
      if (type == DHCP_ACK) 								// 接收到DHCP服务器发出的ACK包
      {
        reset_DHCP_time();									// 重置DHCP超时时间和重连次数
        if (check_leasedIP()) 							// 租用IP并检查租用的IP地址是否有效，保存租用的IP
        {
#ifdef DHCP_DEBUG					
          printf("state : STATE_DHCP_LEASED\r\n");
#endif		
          dhcp_state = STATE_DHCP_LEASED;		// DHCP状态标志位置为STATE_DHCP_LEASED
          return DHCP_RET_UPDATE;						// 返回DHCP_RET_UPDATE，DHCP成功
        } 
        else 
        {
#ifdef DHCP_DEBUG					
...
#endif				
          dhcp_state = STATE_DHCP_DISCOVER;	// 否则DHCP状态标志位置为STATE_DHCP_DISCOVER，重新获取
          return DHCP_RET_CONFLICT;					// 返回DHCP_RET_CONFLICT
        }
      }
      else if (type == DHCP_NAK) 						// 接收到DHCP服务器发出的NAK包
      {
        reset_DHCP_time();									// 重置DHCP超时时间和重连次数
        dhcp_state = STATE_DHCP_DISCOVER;		// DHCP状态标志位置为STATE_DHCP_DISCOVER，重新获取
#ifdef DHCP_DEBUG				
        printf("state : STATE_DHCP_DISCOVER\r\n");
#endif			
      }
      ...
      break;
		...
		// 处于租用状态的IP地址到了租用时限的一半时，需要再次向DHCP服务器申请续租
    case STATE_DHCP_LEASED :								
      if ((dhcp_lease_time != 0xffffffff) && (dhcp_time>(dhcp_lease_time/2))) 
      {
        ...
        DHCP_XID++;
        send_DHCP_REQUEST();								// 再次向DHCP服务器发送请求
        dhcp_state = STATE_DHCP_REREQUEST;	// DHCP状态标志位置为STATE_DHCP_REREQUEST
#ifdef DHCP_DEBUG
    ...
    case STATE_DHCP_REREQUEST :				
      if (type == DHCP_ACK) 								// 接收到DHCP服务器发出的ACK包
      {
        if(memcmp(OLD_SIP,GET_SIP,4)!=0)		// 比较之前的IP和DHCP获取的IP是否相同，若不同，分别打印出来
        {
#ifdef DHCP_DEBUG
...
#endif
          return DHCP_RET_UPDATE;						// 返回DHCP_RET_UPDATE，再次租用成功，IP地址更换
        }
#ifdef DHCP_DEBUG
        else																// 相同
        {
...
#endif
        reset_DHCP_time();									// 重置DHCP超时时间和重连次数
        dhcp_state = STATE_DHCP_LEASED;			// DHCP状态标志位置为STATE_DHCP_LEASED，仍使用之前租用的IP
      } 
      else if (type == DHCP_NAK) 						// 接收到DHCP服务器发出的NAK包，续租失败
      {
        reset_DHCP_time();									// 重置DHCP超时时间和重连次数
        dhcp_state = STATE_DHCP_DISCOVER;		// DHCP状态标志位置为STATE_DHCP_DISCOVER，重新获取
#ifdef DHCP_DEBUG
        ...
        check_DHCP_Timeout();								// 重置DHCP超时时间和重连次数
      break;
			default :
				dhcp_state = STATE_DHCP_READY;			// DHCP状态标志位置为STATE_DHCP_READY
			break;
  }
  
  if (DHCP_timeout == 1)										// DHCP过程超时
  {
    dhcp_state = STATE_DHCP_READY;					// DHCP状态标志位置为STATE_DHCP_READY
    return DHCP_RET_TIMEOUT;								// 返回DHCP_RET_TIMEOUT
  }
  return DHCP_RET_NONE;											// 返回DHCP_RET_NONE
}
...
{  
  uint8 ip_0[4]={0,};
  DHCP_XID = 0x12345678;																// DHCP discover封包编号
  
  memset(OLD_SIP,0,sizeof(OLD_SIP));										// 将上次获取的IP地址用0.0.0.0覆盖  
  memset(DHCP_SIP,0,sizeof(DHCP_SIP));									// DNS地址设为0.0.0.0
  memset(DHCP_REAL_SIP,0,sizeof(GET_SN_MASK));					// DHCP服务器地址设为0.0.0.0

  iinchip_init();																				// 内部寄存器初始化

  setSHAR(ConfigMsg.mac);																// MAC设置
  setSUBR(ip_0);																				// 子网掩码设置为0.0.0.0
  setGAR(ip_0);																					// 网关设置为0.0.0.0
  setSIPR(ip_0);																				// 源IP设置为0.0.0.0
  
  sysinit(txsize, rxsize); 															// W5500发送、接收缓存配置

  dhcp_state = STATE_DHCP_READY;												// DHCP状态标志位清0
#ifdef DHCP_DEBUG
